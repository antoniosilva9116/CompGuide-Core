...
public class MedicationWebService extends AbstractWebService {
	...
	/**
	 * Method to retrieve medication's information. The request must contain a
	...
	public Response medicationDetails(@PathParam("id") String idmedication) {
		...
		try {
			...
			medicationBean = MedicationManager.getInstance()
					.getMedicationBeanById(idmedication);
		} catch (Exception e) {
			return Response
					.serverError()
					.entity("Invalid Medication Id. Please send a integer number")
					.build();
		}
		if (medicationBean != null) {
			medicationJson = Medication.fromBean(medicationBean).toJson();
			return Response.ok(medicationJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 *            The new active ingredient of the medication
	 * @param dosage
	 *            The dosage of the medication
	 * @param pharmaceuticalForm
	 *            The new dosage of the medication
	 * @param posology
	 *            The posology of the medication
	 * @param description
	 *            The description of the medication
	 * @param idtask
	 *            The task of the medication
	 * @param identifier
	 */
	@PUT
	@Path("/add")
	...
	public Response addMedication(@FormParam("name") String name,
		...
		if (!validParam(name) || !validParam(pharmaceuticalForm)
				|| !validParam(posology) || !validParam(identifier)
				|| !validParam(description) || !validParam(activeIngredient)
				|| !validParam(dosage)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		MedicationBean medicationBean = MedicationManager.getInstance()
				.createMedicationBean();
		try {

			medicationBean.setIdtask(Long.parseLong(idtask));
		} catch (Exception e) {
			return Response.serverError()
					.entity("Invalid idtask Id. Please send a integer number")
					.build();
		}
