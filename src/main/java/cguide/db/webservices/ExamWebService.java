...
public class ExamWebService extends AbstractWebService {
	...
	/**
	 * Method to retrieve exam's information. The request must contain a Cookie
	...
	public Response examDetails(@PathParam("id") String idexam) {
		...
		try {
			...
			return Response.serverError()
					.entity("Invalid Exam Id. Please send a integer number")
					.build();
		}
		if (examBean != null) {
			examJson = Exam.fromBean(examBean).toJson();
			return Response.ok(examJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 *
	 * @param name
	 *            The new name of the exam
	 * @param description
	 *            The new description of the exam
	 * @param idtask
	 *            The Guideline Exec ID of the exam
	 * @param identifier
	 */
	@PUT
	@Path("/add")
