...
public class ObservationWebService extends AbstractWebService {
	...
	public Response observationDetails(@PathParam("id") String idobservation) {
		...
		try {
			...
			observationBean = ObservationManager.getInstance()
					.getObservationBeanById(idobservation);
		} catch (Exception e) {
			return Response
					.serverError()
					.entity("Invalid Observation Id. Please send a integer number")
					.build();
		}
		if (observationBean != null) {
			observationJson = Observation.fromBean(observationBean).toJson();
			return Response
					.ok(observationJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 *
	 * @param parameter
	 *            The new parameter of the observation
	 * @param unit
	 *            The new unitof the observation
	 * @param parameterIdentifier
	 *            The parameterIdentifier of the observation
	 * @param variableName
	 *            The new variableName of the observation
	 * @param parameterValue
	 *            The parameterValue of the observation
	 * @param identifier
	 *            The task of the observation
	 * @param isnumeric
	 *            The isnumeric check
	 * @param idtask
	 *            The idtask id
	...
	public Response addObservation(@FormParam("parameter") String parameter,
		...
		if (!validParam(parameter) || !validParam(isnumeric)
				|| !validParam(parameterValue) || !validParam(identifier)
				|| !validParam(unit) || !validParam(idtask)
				|| !validParam(parameterIdentifier)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}

		ObservationBean observationBean = ObservationManager.getInstance()
				.createObservationBean();
		try {
		...
		try {
			...
			return Response
					.serverError()
					.entity("Invalid isnumeric Boolean. Please send a boolean (True/False)")
					.build();
		}
