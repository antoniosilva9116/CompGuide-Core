...
/**
 * Created with IntelliJ IDEA. Patient: tiago Date: 31-07-2013 Time: 16:47 To
...
public class PatientWebService extends AbstractWebService {
	...
	/**
	 * Method to retrieve action's information. The request must contain a
	 * Cookie Header with a valid auth-token and the information will only be
	 * delivered if the patient has permissions to access it
	 * 
	 * @param nutente
	 *            The patientname of the patient
	...
	public Response patientDetails(@PathParam("nutente") String nutente) {
		...
		try {
			patientBean = PatientManager.getInstance().getPatientBeanByNutente(
					nutente);
		} catch (Exception e) {
		...
		if (patientBean != null) {
			...
			return Response.ok(patientJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	...
	/**
	 * This method receives information about a given patient to insert in the
	 * system
	 *
	 * @param nutente
	 *            The new nutente of the patient
	 * @param name
	 *            The patientname of the patient
	 * @param lastname
	 *            The new familyName of the patient
	 * @param birthdate
	 *            The new birth date of the patient
	 * @param address
	 *            The Address of the patient
	 * @param phone
	 *            The new mobile phone of the patient
	 * @param homephone
	 *            The new phone of the patient
	 * @param email
	 *            The new email of the patient
	 */
	@PUT
	@Path("/add/")
	...
	public Response addPatient(@FormParam("nutente") String nutente,
		...
		if (!validParam(nutente) || !validParam(name) || !validParam(lastname)
				|| !validParam(birthdate) || !validParam(address)
				|| !validParam(phone)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		...
		try {
			if (PatientManager.getInstance().isPatient(nutente))
				return Response.ok("Nutente allready exists.").build();

		} catch (DAOException e) {
			e.printStackTrace(); // To change body of catch statement use File |
									// Settings | File Templates.
		}
		PatientBean patientBean = PatientManager.getInstance()
				.createPatientBean();
	 ...
	 *
	 * @param nutente
	 *            The nutente of the patient to remove
	 */
	@DELETE
	...
	public Response deleteUser(@PathParam("nutente") String nutente) {
		...
		int deleted;
		try {
			deleted = PatientManager.getInstance().deletePatientBeanByNutente(
					nutente);
		} catch (DAOException e) {
			return Response.serverError().build();
		}
