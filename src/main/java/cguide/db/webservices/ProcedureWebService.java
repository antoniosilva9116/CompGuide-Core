...
public class ProcedureWebService extends AbstractWebService {
	...
	/**
	 * Method to retrieve procedure's information. The request must contain a
	...
	public Response procedureDetails(@PathParam("id") String idprocedure) {
		...
		try {
			...
			procedureBean = ProcedureManager.getInstance()
					.getProcedureBeanByID(idprocedure);
		} catch (Exception e) {
			return Response
					.serverError()
		...
		if (procedureBean != null) {
			...
			return Response.ok(procedureJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 * @param description
	 *            The new description of the procedure
	 * @param idtask
	 *            The Guideline Exec ID of the procedure
	 * @param identifier
	 */
	@PUT
	@Path("/add")
	...
	public Response addProcedure(@FormParam("idtask") String idtask,
		...
		ProcedureBean procedureBean = ProcedureManager.getInstance()
				.createProcedureBean();
		try {
