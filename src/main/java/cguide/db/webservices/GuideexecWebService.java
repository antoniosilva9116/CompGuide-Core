...
public class GuideexecWebService extends AbstractWebService {
	...
	public Response guideexecDetails(@PathParam("id") String idguideexec) {
		...
		try {
			...
			return Response.serverError()
					.entity("Invalid  Id. Please send a integer number")
					.build();
		}
		if (guideexecBean != null) {
			guideexecJson = Guideexec.fromBean(guideexecBean).toJson();
			return Response.ok(guideexecJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	...
	public Response addGuideexec(@FormParam("idguideline") String idguideline,
		...
		if (!validParam(idguideline) || !validParam(iduser)
		...
		try {
			...
			return Response.serverError()
					.entity("Invalid parser Id. Please send a integer number")
					.build();
		}
		try {
			...
			return Response.serverError()
					.entity("Invalid user Id. Please send a integer number")
					.build();
		}
		if (validParam(nextTasks)) {
			...
			String firstPlan = guidelineHandler
					.getClinicalPracticeGuideline(guidelinebean.getIdentifier())
					.getPlan().getId();
		...
		if (validParam(completed)) {
			try {
				...
				return Response
						.serverError()
						.entity("Invalid Completed value. Please send a BOOLEAN ")
						.build();
			}
		...
		try {
			...
			return Response.serverError()
					.entity("Invalid patient Id. Please send a integer number")
					.build();
		}
		...
		if (key > 0) {
			...
			return Response.ok(
					"Guideexec #" + key + "# successfully registered").build();

		} else {
			return Response.serverError().entity("Invalid Patient Number")
					.build();
		}
	}
	...
	public Response guideexecActive(@PathParam("user") String user) {
		...
		try {
			...
			guideexecList = GuideexecManager.getInstance()
					.getGuidelineListActive(user);
		} catch (Exception e) {
			return Response.serverError()
					.entity("Invalid  Id. Please send a integer number")
					.build();
	...
	public Response deleteGuideexec(@PathParam("idguideexec") String idguideexec) {
		...
		try {
			deleted = GuideexecManager.getInstance().deleteGuideexecBeanById(
					Long.valueOf(idguideexec));
		} catch (Exception e) {
			return Response
					.serverError()
					.entity("Invalid GuideExec Id. Please send a integer number")
					.build();
		}
		if (deleted > 0) {
