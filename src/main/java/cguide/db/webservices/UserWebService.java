...
/**
 * Created with IntelliJ IDEA. User: tiago Date: 18-07-2013 Time: 14:47 To
...
public class UserWebService extends AbstractWebService {
	...
	@Path("/whoami")
	@StatusCodes({
			@ResponseCode(code = 403, condition = "If the user is not logged in!"),
	...
	public Response whoAmI() {
		...
		return Response.ok(User.fromBean(requestOwner).toJson(),
	...
	public Response userExists(@QueryParam("username") String username)
		...
		return Response.ok("{exists:" + userExists.toString() + "}",
				MediaType.APPLICATION_JSON_TYPE).build();
	}

	/**
	 * Method to be called to check if a username exists (repeated method, but
	 * the username is received in path)
	 * 
	 * @param username
	 *            Username to check
	 * @return A json object like "{exists:true}"
	...
	/**
	 * Method used for the applications to obtain a token to perform requests on
	 * behalf of the user
	 *
	 * @param username
	 ...
	 * @return An auth-token for the application to authenticate the next
	 *         requests
	 */
	...
	public Response userAuthentication(@FormParam("username") String username,
		...
		boolean validUser;
		AutenticacaoBean autenticacaoBean = new AutenticacaoManager()
				.getInstance().createAutenticacaoBean();
		UserBean user = UserManager.getInstance().getUserBeanByUsername(
				username);
		validUser = !(user == null);
		if (validUser) {
			validUser = user.getUsername().equals(username)
					&& user.getPassword().equals(password);
			if (validUser) {
				if ((user.getActive() != null) && (!user.getActive())) {
					return Response
							.status(Response.Status.FORBIDDEN)
							.entity("User must confirm the email account before login")
							.build();
				}
			}
			// prepare auth token
			autenticacaoBean = AutenticacaoManager.getInstance()
					.createAutenticacaoBean();
		...
		if (!validUser) {
			return Response.status(Response.Status.FORBIDDEN)
					.entity("User credentials are invalid").build();
		} else {
			NewCookie authCookie = new NewCookie("auth-token",
					autenticacaoBean.getAuth(), "/", null, null,
					5 * 24 * 60 * 60, false);
			// NewCookie validityCookie = new NewCookie("token-validity",
			// autenticacaoBean.getDuracao().toString());
			return Response
					.ok()
					.cookie(authCookie)
					.entity("User credentials are valid and a new cookie was returned")
					.build();
		}
	}
	 ...
	 * userDetailsOld(@QueryParam("username") String username) { return
	 * userDetails(username); }
	 */

	/**
	 * Method to retrieve user's information. The request must contain a Cookie
	...
	public Response userDetails(@PathParam("username") String username) {
		...
		if (requestOwner.getUsername().equals("username")) {
			...
			return Response.ok(userJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
			try {
				if (isAdmin() && UserManager.getInstance().isUser(username)) {
					...
					UserBean user = UserManager.getInstance()
							.getUserBeanByUsername(username);
					if (user != null) {
						utilizadorBean = user;
						userJson = User.fromBean(utilizadorBean).toJson();
	...
	/**
	 * Method to be called when a user receives an email with the confirmation
	 * url
	 *
	 ...
	 * @param key
	 *            Key to prove that the email was received
	 */
	...
	public Response userActivation(@QueryParam("username") String username,
			@QueryParam("key") String key) {
		if (!validParam(username) || !validParam(key)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		...
		try {
			...
			return Response.serverError()
					.entity("Error while processing activation request")
					.build();
		}
		if (validUser) {
			return Response.ok("Email was validated. Login is now allowed")
					.build();
		} else {
			return Response
					.status(Response.Status.NOT_FOUND)
					.entity("Resource not found. Check your link or register again")
					.build();
		}
	}

	/**
	 * Method to be called when a user forgets the password. This will create a
	 * new password and sent it to the email.
	 *
	 * @param username
	 *            The username of the user
	 */
	...
	public Response userForgot(@QueryParam("username") String username) {
		...
		try {
			...
			if (user != null) {
				...
				UserManager.getInstance().update(user);
				Mailer.sendPasswordRecoveryEmail(user.getEmail(),
						user.getPassword(), user.getUsername());
			}
		} catch (Exception e) {
			return Response.serverError()
					.entity("Error while processing forgot request").build();
		}
		if (validUser) {
			return Response.ok("An email has been sent with a password!")
					.build();
		} else {
			return Response
					.status(Response.Status.NOT_FOUND)
	 ...
	 * @param lastname
	 *            The new familyName of the user
	 * @param photo
	 *            The new url to the photo of the user
	 * @param phone
	 *            The new mobile phone of the user
	 * @param type
	 *            The type of the user (This can only be setted by an
	 *            administrator)
	 * @param homephone
	 ...
	 * @param birthdate
	 *            The new birth date of the user
	 */
	@PUT
	@Path("/user/{username}")
	@Consumes(MediaType.APPLICATION_FORM_URLENCODED)
	...
	public Response addNewUser(@PathParam("username") String username,
		...
		if (!validParam(username) || !validParam(password) || !validParam(name)
				|| !validParam(email)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		...
		if (isAdmin() && validParam(type)) {
			...
			try {
				...
				return Response.status(Response.Status.BAD_REQUEST)
						.entity("User type is wrong: " + type).build();
			}
		...
		try {
			Mailer.sendActivationMail(email, userBean.getActivationkey()
					.toString(), userBean.getUsername(), userBean.getPassword());
		} catch (Exception e) {
			// TODO remove user?
			return Response.serverError()
					.entity("Unable to send confirmation mail").build();
		}
		return Response
				.ok("User successfully registered. A confirmation email was sent..")
				.build();
	}
	...
	public Response updateUser(@PathParam("username") String username,
			...
			@FormParam("name") String name, @FormParam("email") String email,
			@FormParam("lastname") String lastname,
			@FormParam("photo") String photo, @FormParam("phone") String phone,
			@FormParam("type") @DefaultValue("user") String type,
			@FormParam("homephone") String homephone,
			@FormParam("address") String address,
			@FormParam("birthdate") String birthdate) {
		if (requestOwner == null) {
			return Response.status(Response.Status.FORBIDDEN)
					.entity("Must be logged in").build();
		}
		...
		if (requestOwner.getUsername().equals(username)) {
			...
			try {
				...
				return Response.serverError()
						.entity("Error while getting user from database")
						.build();
			}
		} else {
			return Response.status(Response.Status.FORBIDDEN)
					.entity("User don't have permission to edit other users")
					.build();
		}

		if (!validParam(username) || !validParam(email)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		if (validParam(birthdate)) {
			try {
				...
				return Response
						.serverError()
						.entity("Invalid birthdate date. Please send a timestamp in miliseconds")
						.build();
			}
		}
		if (validParam(address)) {
		...
		if (!userToEdit.getEmail().equals(email)) {
			...
			try {
				Mailer.sendEmailChangedMail(email,
						userToEdit.getActivationkey(), userToEdit.getUsername());
			} catch (Exception e) {
				return Response.serverError().build();
			}
		}
	...
	public Response deleteUser(@PathParam("username") String username) {
		...
		int deleted;
		try {
			deleted = UserManager.getInstance().deleteUserBeanByUsername(
					username);
		} catch (DAOException e) {
			return Response.serverError().build();
		}
	...
	protected Boolean isUser(String username) {
		return requestOwner != null
				&& requestOwner.getUsername().equals(username);
	}
}
