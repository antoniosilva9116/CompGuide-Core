...
public class GuidelineWebService extends AbstractWebService {
	...
	public Response updateGuideline() {
		...
		for (String identifier : guidelines) {
			try {
				if (GuidelineManager.getInstance().isGuideline(identifier)) {
					...
					cguide.execution.entities.Guideline guideline = guidelineHandler
							.getClinicalPracticeGuideline(identifier);
					guidelineBean.setIdentifier(guideline.getId());
					guidelineBean.setAuthorship(guideline.getAuthorship());
					guidelineBean.setDateofcreation(guideline
							.getDateofcreation());
					guidelineBean.setDateofupdate(guideline.getDateofupdate());
					guidelineBean
							.setGuidelinename(guideline.getGuidelinename());
					guidelineBean
							.setVersionnumber(guideline.getVersionnumber());
					guidelineBean.setDescription(guideline
		...
		return Response.ok(
				"Guidelines" + guidelines.toString()
						+ " successfully updated/added.").build();
	}

	/**
	 * Method to retrieve parser's information. The request must contain a
	...
	public Response guidelineDetails(@PathParam("id") String idguideline) {
		...
		try {
			...
			guidelineBean = GuidelineManager.getInstance()
					.getGuidelineBeanByID(idguideline);
		} catch (Exception e) {
			return Response
					.serverError()
					.entity("Invalid Guideline Id. Please send a integer number")
					.build();
		}
		if (guidelineBean != null) {
			guidelineJson = Guideline.fromBean(guidelineBean).toJson();
			return Response.ok(guidelineJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 *            The new dateOfCreation of the parser
	 * @param dateOfUpdate
	 *            The new versionNumber of the parser
	 * @param versionNumber
	 *            The Guideline Exec ID of the parser
	 * @param guidelineName
	 *            The new versionNumber of the parser
	 * @param authorship
	 *            The Guideline Exec ID of the parser
	...
	public Response addGuideline(
			@FormParam("dateOfCreation") String dateOfCreation,
		...
		if (!validParam(dateOfCreation) || !validParam(dateOfCreation)
				|| !validParam(guidelineName) || !validParam(identifier)
				|| !validParam(authorship) || !validParam(versionNumber)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		GuidelineBean guidelineBean = GuidelineManager.getInstance()
				.createGuidelineBean();
		try {
			...
			return Response
					.serverError()
					.entity("Invalid date. Please send a date in the format yyyy-mm-dd hh:mm:ss")
					.build();
		}
