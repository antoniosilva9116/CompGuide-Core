...
public class NonmedicationWebService extends AbstractWebService {
	...
	/**
	 * Method to retrieve nonmedication's information. The request must contain
	 * a Cookie Header with a valid auth-token and the information will only be
	 * delivered if the user has permissions to access it
	 * 
	 * @param idnonmedication
	...
	public Response nonmedicationDetails(@PathParam("id") String idnonmedication) {
		...
		try {
			...
			nonmedicationBean = NonmedicationManager.getInstance()
					.getNonmedicationBeanByID(idnonmedication);
		} catch (Exception e) {
			return Response
					.serverError()
		...
		if (nonmedicationBean != null) {
			nonmedicationJson = Nonmedication.fromBean(nonmedicationBean)
					.toJson();
			return Response.ok(nonmedicationJson,
	 ...
	 *
	 * @param name
	 *            The new name of the nonmedication
	 * @param description
	 *            The new description of the nonmedication
	 * @param idtask
	 *            The Guideline Exec ID of the nonmedication
	 * @param identifier
	 */
	@PUT
	@Path("/add")
	...
	public Response addNonmedication(@FormParam("idtask") String idtask,
		...
		if (!validParam(idtask) || !validParam(name) || !validParam(identifier)
				|| !validParam(description)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		NonmedicationBean nonmedicationBean = NonmedicationManager
				.getInstance().createNonmedicationBean();
		try {
