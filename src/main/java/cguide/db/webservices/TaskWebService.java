...
public class TaskWebService extends AbstractWebService {
	...
	public Response taskDetails(@PathParam("id") String idtask) {
		...
		try {
			...
			return Response.serverError()
					.entity("Invalid Task Id. Please send a integer number")
					.build();
		}
		if (taskBean != null) {
			taskJson = Task.fromBean(taskBean).toJson();
			return Response.ok(taskJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 *            The new taskidentifier of the task
	 * @param taskDescription
	 *            The new active ingredient of the task
	 * @param taskFormat
	 *            The taskformat of the task
	 * @param taskType
	 *            The new taskformat of the task
	 * @param idguideexec
	...
	public Response addTask(@FormParam("taskIdentifier") String taskidentifier,
		...
		if (!validParam(taskidentifier) || !validParam(taskType)
				|| !validParam(taskDescription) || !validParam(taskPlan)
				|| !validParam(nextTask) || !validParam(taskFormat)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
