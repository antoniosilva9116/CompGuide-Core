...
public class FormulaWebService extends AbstractWebService {
	...
	/**
	 * Method to retrieve formula's information. The request must contain a
	...
	public Response formulaDetails(@PathParam("id") String idformula) {
		...
		try {
			...
			formulaBean = FormulaManager.getInstance().getFormulaBeanByID(
					idformula);
		} catch (Exception e) {
			return Response.serverError()
					.entity("Invalid Event Id. Please send a integer number")
					.build();
		}
		if (formulaBean != null) {
			formulaJson = Formula.fromBean(formulaBean).toJson();
			return Response.ok(formulaJson, MediaType.APPLICATION_JSON_TYPE)
					.build();
		} else {
	 ...
	 *
	 * @param parameter
	 *            The new name of the formula
	 * @param description
	 *            The new description of the formula
	 * @param idtask
	 *            The Guideline Exec ID of the formula
	 * @param identifier
	 *            The formula id
	 */
	@PUT
	...
	public Response addFormula(@FormParam("idtask") String idtask,
		...
		if (!validParam(idtask) || !validParam(parameter)
				|| !validParam(identifier) || !validParam(description)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		FormulaBean formulaBean = FormulaManager.getInstance()
				.createFormulaBean();
		try {
