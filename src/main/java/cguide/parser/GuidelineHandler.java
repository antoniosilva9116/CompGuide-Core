...
public class GuidelineHandler {
	...
	public ArrayList<String> getGuidelines() {
		...
		OWLClass owlClass = this.factory.getOWLClass(IRI.create(baseURL
	...
	public Guideline getClinicalPracticeGuideline(String name) {
		...
		Map<OWLDataPropertyExpression, Set<OWLLiteral>> dataPropertyValues = individual
				.getDataPropertyValues(manager.getOntology(IRI.create(this
						.getBaseURL())));
		Map<OWLObjectPropertyExpression, Set<OWLIndividual>> objectPropertyValues = individual
				.getObjectPropertyValues(manager.getOntology(IRI.create(this
						.getBaseURL())));
		System.out.println("\n\n\n......LOADING Guideline " + name);
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
				.entrySet()) {
			...
			for (OWLLiteral literal : s) {
				if (field.equals("dateOfLastUpdate")) {
					guideline.setDateofupdate(parseDateTime(literal
							.getLiteral()));
				}
				if (field.equals("dateOfCreation")) {
					guideline.setDateofcreation(parseDateTime(literal
							.getLiteral()));
				}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
				.entrySet()) {
			String field = parseIRI(entry.getKey().toString());
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasPlan")) {
					guideline
							.setPlan(this.getPlan(parseIRI(literal.toString())));

				}
				if (field.equals("hasScope")) {
					guideline.setScope(this.getScope(parseIRI(literal
							.toString())));
				}
			}
		}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				...
				if (field.equals("hasScopeConditionSet")) {
					scope.addConditionSet(this.getConditionSet(parseIRI(literal
							.toString())));
				}
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
			...
			for (OWLLiteral literal : s) {
				if (field.equals("conditionSetCounter")) {
					conditionSet.addConditionSetCounter(Integer.valueOf(literal
							.getLiteral()));
				}
			}
		}
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasComparisonOperator")) {
					conditionSet.addComparisonOperator(parseIRI(literal
							.toString()));
				}
				if (field.equals("hasCondition")) {
					conditionSet.addCondition(this
							.getCondition(parseIRI(literal.toString())));
				}
	...
	public Condition getCondition(String name) {
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasComparisonOperator")) {
					condition
							.setComparisonOperator(parseIRI(literal.toString()));
				}
				if (field.equals("hasTemporalRestriction")) {
					condition
							.addTemporalRestriction(this
									.getTemporalRestriction(parseIRI(literal
											.toString())));
				}
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
			...
			for (OWLLiteral literal : s) {
				if (field.equals("maxTemporalRestrictionValue")) {
					temporalRestriction.setMaxRestrictionValue(Double
							.valueOf(literal.getLiteral()));
				}
				if (field.equals("minTemporalRestrictionValue")) {
					temporalRestriction.setMinRestrictionValue(Double
							.valueOf(literal.getLiteral()));
				}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasTemporalOperator")) {
					temporalRestriction.setTemporalOperator(parseIRI(literal
							.toString()));
				}
				if (field.equals("hasTemporalUnit")) {
					temporalRestriction.setTemporalUnit(parseIRI(literal
							.toString()));
				}
			}
		}
	...
	public ClinicalAction getClinicalAction(String name) {
		...
		if (type.equals("MedicationRecommendation")) {
			ClinicalAction medicationRecommendation = this
					.getMedicationRecommendation(name);
			medicationRecommendation.setActionType(type);
			return medicationRecommendation;
		}
		if (type.equals("NonMedicationRecommendation")) {
			ClinicalAction nonMedicationRecommendation = this
					.getNonMedicationRecommendation(name);
	...
	private Exam getExam(String name) {
		...
		if (exam.getDescription() != null && exam.getName() != null) {
			return exam;
		} else {
			return null;
		}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasVariableParameter")) {
					formula.addParameter(this.getParameter(parseIRI(literal
							.toString())));
				}
				if (field.equals("hasResultParameter")) {
					formula.setResult(this.getParameter(parseIRI(literal
							.toString())));
				}
			}
		}
	...
	private NonMedicationRecommendation getNonMedicationRecommendation(
			String name) {
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
			...
			for (OWLLiteral literal : s) {
				if (field.equals("nonMedicationRecommendationDescription")) {
					nonMedicationRecommendation.setDescription(literal
							.getLiteral());
				}
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
			...
			for (OWLLiteral literal : s) {
				if (field.equals("medicationRecommendationDescription")) {
					medicationRecommendation.setDescription(literal
							.getLiteral());
				}
				if (field.equals("activeIngredient")) {
					medicationRecommendation.setActiveIngredient(literal
							.getLiteral());
				}
				...
				if (field.equals("pharmaceuticalForm")) {
					medicationRecommendation.setPharmaceuticalForm(literal
							.getLiteral());
				}
	...
	private Procedure getProcedure(String name) {
		...
		if (procedure.getDescription() != null && procedure.getName() != null) {
			return procedure;
		} else {
			return null;
		}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasTriggerCondition")) {
					end.addTriggerCondition(this
							.getTriggerCondition(parseIRI(literal.toString())));
				}
			}
		}
		return end;
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasPreCondition")) {
					plan.addPreCondition(this.getPreCondition(parseIRI(literal
							.toString())));
				}
				if (field.equals("hasTriggerCondition")) {
					plan.addTriggerCondition(this
							.getTriggerCondition(parseIRI(literal.toString())));
				}
				...
				if (field.equals("hasDuration")) {
					plan.setDuration(this.getDuration(parseIRI(literal
							.toString())));
				}
				if (field.equals("hasPeriodicity")) {
					plan.setPeriodicity(this.getPeriodicity(parseIRI(literal
							.toString())));
				}
	...
	private Question getQuestion(String name) {
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasPreCondition")) {
					question.addPreCondition(this
							.getPreCondition(parseIRI(literal.toString())));
				}
				if (field.equals("hasTriggerCondition")) {
					question.addTriggerCondition(this
							.getTriggerCondition(parseIRI(literal.toString())));
				}
				if (field.equals("hasParameter")) {
					question.addParameter(this.getParameter(parseIRI(literal
							.toString())));
				}
				if (field.equals("alternativeTask")) {
	...
	private Decision getDecision(String name) {
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasPreCondition")) {
					decision.addPreCondition(this
							.getPreCondition(parseIRI(literal.toString())));
				}
				if (field.equals("hasTriggerCondition")) {
					decision.addTriggerCondition(this
							.getTriggerCondition(parseIRI(literal.toString())));
				}
				if (field.equals("hasOption")) {
					decision.addOption(this.getOption(parseIRI(literal
							.toString())));
				}
				if (field.equals("alternativeTask")) {
	...
	private Action getAction(String name) {
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasPreCondition")) {
					action.addPreCondition(this
							.getPreCondition(parseIRI(literal.toString())));
				}
				if (field.equals("hasTriggerCondition")) {
					action.addTriggerCondition(this
							.getTriggerCondition(parseIRI(literal.toString())));
				}
				if (field.equals("hasClinicalActionType")) {
					action.addClinicalAction(this
							.getClinicalAction(parseIRI(literal.toString())));
				}
				if (field.equals("alternativeTask")) {
				...
				if (field.equals("hasOutcome")) {
					action.setOutcome(this.getOutcome(parseIRI(literal
							.toString())));
				}
				if (field.equals("syncTask")) {
				...
				if (field.equals("hasPeriodicity")) {
					action.setPeriodicity(this.getPeriodicity(parseIRI(literal
							.toString())));
				}
				if (field.equals("stopConditionTask")) {
					action.addClinicalTask(parseIRI(literal.toString()));
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasPreConditionSet")) {
					preCondition.addPrecondition(this
							.getConditionSet(parseIRI(literal.toString())));
				}
			}
		}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasOutcomeConditionSet")) {
					outcome.addOutcome(this.getConditionSet(parseIRI(literal
							.toString())));
				}
			}
		}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasTriggerConditionSet")) {
					triggerCondition.addTriggerCondition(this
							.getConditionSet(parseIRI(literal.toString())));
				}
			}
		}
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
			...
			for (OWLLiteral literal : s) {
				if (field.equals("maxDurationValue")) {
					duration.setMaxDurationValue(Double.valueOf(literal
							.getLiteral()));
				}
				if (field.equals("minDurationValue")) {
					duration.setMinDurationValue(Double.valueOf(literal
							.getLiteral()));
				}
		...
		for (Map.Entry<OWLDataPropertyExpression, Set<OWLLiteral>> entry : dataPropertyValues
			...
			for (OWLLiteral literal : s) {
				if (field.equals("periodicityValue")) {
					periodicity.setMinPeriodicityValue(Double.valueOf(literal
							.getLiteral()));
					periodicity.setMaxPeriodicityValue(Double.valueOf(literal
							.getLiteral()));
				}
				if (field.equals("minPeriodicityValue")) {
					periodicity.setMinPeriodicityValue(Double.valueOf(literal
							.getLiteral()));
				}
				if (field.equals("maxPeriodicityValue")) {
					periodicity.setMaxPeriodicityValue(Double.valueOf(literal
							.getLiteral()));
				}
				if (field.equals("repetitionValue")) {
					periodicity.setRepetitionValue(Integer.valueOf(literal
							.getLiteral()));
				}
			}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				...
				if (field.equals("hasStopConditionSet")) {
					periodicity.addStopConditionSet(this
							.getConditionSet(parseIRI(literal.toString())));
				}
		...
		for (Map.Entry<OWLObjectPropertyExpression, Set<OWLIndividual>> entry : objectPropertyValues
			...
			for (OWLIndividual literal : s) {
				if (field.equals("hasOptionConditionSet")) {
					option.addOptionConditionSet(this
							.getConditionSet(parseIRI(literal.toString())));
				}

			}
		}
		if (option.getValue() != null && option.getOptionParameter() != null
				&& option.getUnit() != null
