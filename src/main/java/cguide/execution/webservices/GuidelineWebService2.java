...
public class GuidelineWebService2 extends AbstractWebService {
	...
	public Response getTasks(@FormParam("request") String request) {
		...
		if (!validParam(request)) {
			return Response.status(Response.Status.NOT_ACCEPTABLE)
					.entity("Mandatory params are missing").build();
		}
		if (!isUser()) {
			return Response.status(Response.Status.FORBIDDEN)
					.entity("Private information").build();
		}
		...
		if (clinicalTaskList.size() > 0) {
			return Response.ok(clinicalTaskList.toJson(),
	...
	/**
	 * Method to retrieve task's information. The request must contain a Cookie
	 * Header with a valid auth-token and the information will only be delivered
	 * if the user has permissions to access it
	 * 
	 * @param request
	 *            The usertaskidentifier of the user
	...
	public Response nextTask(@FormParam("request") String request) {
		...
		ClinicalTask task = guidelineHandler.getClinicalTask(taskRequest
				.getTaskQuadruple().getTask());
		...
		// LOAD SYNCTASK
		GuideexecBean guideexecBean = GuideexecManager.getInstance()
				.getGuideexecBeanByID(taskRequest.getGuideexec());
		TaskController taskController = gson.fromJson(
				guideexecBean.getNextTasks(), TaskController.class);
		if (guideexecBean == null) {

			return Response.status(Response.Status.NOT_FOUND)
					.entity("Failed to retrieve Guideline execution from db.")
					.build();

		}
		if (taskController == null) {
			return Response.status(Response.Status.NOT_FOUND)
					.entity("Failed to task Controller from db.").build();
		...
		/*
		 * if(taskController.peekSyncTask()!=null&&!taskController.isSyncable(
		 * taskRequest
		 * .getTaskQuadruple().getId(),taskRequest.getTaskQuadruple().
		 * getTask())){
		 * 
		 * return Response.status(Response.Status.BAD_REQUEST).entity(
		 * "Current task requires other parallel tasks to be completed first."
		 * ).build(); }
		 */
		...
		if (!taskController.contains(taskRequest.getTaskQuadruple())) {
			...
			return Response
					...
					.entity("Task not found in the current guideline execution.")
					.build();

		}
		// Remove alternative tasks

		if (taskController.isAlternativeTask(taskRequest.getTaskQuadruple()
				.getId())) {
			taskController.removeAlternativeTask(taskRequest.getTaskQuadruple()
					.getId());
			// taskController.addTask(taskRequest.getTaskQuadruple());
		}

		TaskQuadruple currentTask = taskRequest.getTaskQuadruple();
		GeneratedTaskBean generatedTaskBean = GeneratedTaskManager
				.getInstance().createGeneratedTaskBean();
		...
		try {
			idtask = String.valueOf(GeneratedTaskManager.getInstance().insert(
					generatedTaskBean));
		} catch (DAOException e) {
			e.printStackTrace(); // To change body of catch statement use File |
									// Settings | File Templates.
		...
		if (task.getTaskType().equals("Plan")) {
			...
			if (plan.getFirstTaskFormat().equals("hasFirstTask")) {
				if (taskController.getSyncTask().peek() != null) {
					...
					taskController.updateTask(taskRequest.getTaskQuadruple(),
							plan.getFirstClinicalTask().get(0));
					...
					if (plan.getFirstClinicalTask().size() > 1) {
						...
						for (String ct : plan.getFirstClinicalTask()) {
							taskController.addTask(new TaskQuadruple(ct, plan
									.getSyncTask(), id, idtask));
							taskController.pushSync(id);
						}

					} else {
						taskController.addTask(new TaskQuadruple(plan
								.getFirstClinicalTask().get(0), plan
								.getFirstClinicalTask().get(0), id, idtask));
					}
		...
		if (task.getTaskType().equals("Decision")) {
			...
			try {
				previousConditions = ObservationManager.getInstance()
						.getParameterByIdguideexec(
								guideexecBean.getIdguideexec());
			} catch (DAOException e) {
			...
			for (Option option : decision.getOptions()) {
				Boolean test = Utils.processDecision(previousConditions,
						option.getOptionConditionSet());

				if (test) {
					processedDecision.setUnit(option.getUnit());
					processedDecision.setOptionParameter(option
							.getOptionParameter());
					processedDecision.setParameterIdentifier(option
							.getOptionIdentifier());
			...
			if (decision.getTaskFormat().equals("parallelTask")) {
				for (String taskname : decision.getClinicalTasks()) {
					taskController.addTask(new TaskQuadruple(taskname, decision
							.getSyncTask(), id, taskRequest.getTaskQuadruple()
							.getPlan()));
			...
			if (decision.getTaskFormat().equals("alternativeTask")) {
				...
				Gson gson1 = new Gson();
				for (String taskname : decision.getClinicalTasks()) {
					ClinicalTask ct = guidelineHandler
							.getClinicalTask(taskname);
					if (Utils.alternateTaskTest(previousConditions,
							ct.getTriggerCondition())) {
						if (taskController.getSyncTask().peek() != null) {

							// update request task
							// in case it was updated already it adds a new task
							// to the next tasks
							taskController.updateTask(
									taskRequest.getTaskQuadruple(), ct.getId());
						} else {

							// adds a new task
							// request task is to be removed after
							taskController.addTask(new TaskQuadruple(
									ct.getId(), ct.getId(), id, taskRequest
											.getTaskQuadruple().getPlan()));
				...
				// if there's more then one selectable task, set it to
				// alternative
				if (counter > 1) {
			...
			if (decision.getTaskFormat().equals("preferenceAlternativeTask")) {
				...
				for (String taskname : decision.getClinicalTasks()) {
					...
					if (taskController.getSyncTask().peek() != null) {
						...
						taskController.addTask(new TaskQuadruple(taskname, id,
								id, taskRequest.getTaskQuadruple().getPlan()));
					}
				}
				// set as alternative task
				taskController.addAlternativeTask(id);
			...
			if (decision.getTaskFormat().equals("nextTask")) {
				taskController.addTask(new TaskQuadruple(decision
						.getClinicalTasks().get(0), decision.getClinicalTasks()
						.get(0), id, taskRequest.getTaskQuadruple().getPlan()));
		...
		if (task.getTaskType().equals("Question")) {
			...
			if (taskRequest.getConditionSet() == null) {
				...
				return Response.status(Response.Status.BAD_REQUEST)
						.entity("Missing parameters for Question task.")
						.build();
			...
			if (format.equals("parallelTask")) {
				...
				for (String taskname : question.getClinicalTasks()) {
					taskController.addTask(new TaskQuadruple(taskname, question
							.getSyncTask(), id, taskRequest.getTaskQuadruple()
							.getPlan()));
			...
			if (format.equals("alternativeTask")) {
				...
				previousConditions.addAll(taskRequest.getConditionSet()
						.getCondition());
				Gson gson1 = new Gson();
			...
			if (format.equals("nextTask")) {
				...
				taskController.addTask(new TaskQuadruple(question
						.getClinicalTasks().get(0), question.getClinicalTasks()
						.get(0), id, taskRequest.getTaskQuadruple().getPlan()));
			}
			...
			for (Condition c : taskRequest.getConditionSet().getCondition()) {
				ObservationBean observationBean = ObservationManager
						.getInstance().createObservationBean();
				...
				if (c.getParameterIdentifier().size() > 0) {
					observationBean.setParameteridentifier(c
							.getParameterIdentifier().get(0));
				}
		...
		if (task.getTaskType().equals("End")) {
			...
			if (taskController.peekPendingPlan() != null) {
				Plan plan = (Plan) guidelineHandler
						.getClinicalTask(GeneratedTaskManager.getInstance()
								.getTaskIdentifierById(
										taskRequest.getTaskQuadruple()
												.getPlan()));

				taskController.removePlan(taskRequest.getTaskQuadruple()
						.getPlan());

				// add a next task to the old plan with ending plan next task
				for (String plantask : plan.getClinicalTasks()) {
					taskController
							.addTask(new TaskQuadruple(plantask, plantask,
									idtask, taskController.peekPendingPlan()));
				}
				// implement for diferent type of tasks, though this should work
				// fine, unless it's alternative which i doubt it can happen
			}
			...
			// still figuring it out
				...
				try {
					...
					return Response.ok(processedTask.toJson(),
							MediaType.APPLICATION_JSON_TYPE).build();
				} catch (DAOException e) {
					e.printStackTrace(); // To change body of catch statement
											// use File | Settings | File
		...
		if (task.getTaskType().equals("Action")) {
			...
			if (format.equals("parallelTask")) {
				for (String taskname : action.getClinicalTasks()) {
					taskController.addTask(new TaskQuadruple(taskname, action
							.getSyncTask(), id, taskRequest.getTaskQuadruple()
							.getPlan()));
				}
				taskController.pushSync(id);
			...
			if (format.equals("alternativeTask")) {
				...
				for (String taskname : action.getClinicalTasks()) {
					ClinicalTask ct = guidelineHandler
							.getClinicalTask(taskname);
					if (Utils.alternateTaskTest(taskRequest.getConditionSet()
							.getCondition(), ct.getTriggerCondition())) {
						if (taskController.getSyncTask().peek() != null) {

							// update request task
							// in case it was updated allready it adds a new
							// task to the next taks
							taskController.updateTask(
									taskRequest.getTaskQuadruple(), ct.getId());
						} else {
							// adds a new task
			...
			if (format.equals("nextTask")) {
				...
				taskController.addTask(new TaskQuadruple(action
						.getClinicalTasks().get(0), action.getClinicalTasks()
						.get(0), id, taskRequest.getTaskQuadruple().getPlan()));
		...
		// test if the execution of this task triggered the completion of a
		// parallel task
		if (taskController.getSyncTask().peek() != null) {
			// if is parallel
			if (taskController.getSyncTask().peek()
					.equals(taskRequest.getTaskQuadruple().getId())) {
				// if the id of the task is the same in the sync queue

				if (taskController.isAllowed(taskRequest.getTaskQuadruple()
						.getId())) {

					// if all tasks are Sync'ed remove the tasks and pop the id
					// fro
					taskController.removeParallelTask(taskRequest
							.getTaskQuadruple().getId(), idtask);
				} else {
			...
			// remove the tasks that are sync'ed but require other tasks to be
			// sync'ed
		...
		try {
			...
			return Response.status(Response.Status.SERVICE_UNAVAILABLE)
					.entity("Failed to update execution").build();
		...
		if (processedDecision != null) {
			processedTask.setDecision(processedDecision);
		}
		return Response.ok(processedTask.toJson(),
				MediaType.APPLICATION_JSON_TYPE).build();
	...
	/**
	 * Method to retrieve observation's information. The request must contain a
	...
	public Response lastTask(@PathParam("idguideexec") String idguideexec) {
		...
		try {
			...
			guideexecBean = GuideexecManager.getInstance()
					.getGuideexecBeanByID(idguideexec);
		} catch (Exception e) {
			return Response
					.serverError()
					.entity("Invalid idguideexec Id. Please send a integer number")
					.build();
		}
		if (guideexecBean != null) {
			TaskController taskController = TaskController
					.fromJson(guideexecBean.getNextTasks());

			processedTask.setTasks(getClinicalTasks(taskController
					.getNextTask()));
			processedTask.setController(taskController);
			return Response.ok(processedTask.toJson(),
					MediaType.APPLICATION_JSON_TYPE).build();
	...
	private ArrayList<ClinicalTask> getClinicalTasks(
			ArrayList<TaskQuadruple> TaskQuadruples) {
