...
public class MailUtils {
	...
	public static Message createActivationEmail(Session mailSession,
			String key, String email, String username, String password)
		...
		mail.setText("Hi "
				...
				+ email
				+ ") to register an account at cguide Portal.\n"
				+ "The acount info is:\n Username = "
				+ username
				+ "\n Password = "
				+ password
				...
				+ " http://localhost:8080/CGuide_war_exploded/activation?username="
				+ username + "&key=" + key);

		return mail;
	}

	public static Message createPasswordChangedEmail(Session mailSession,
			String key, String email, String username)
			throws MessagingException, NamingException,
			UnsupportedEncodingException {
		MimeMessage mail = new MimeMessage(mailSession);
	...
	public static Message createRecoveryEmail(Session mailSession,
			String email, String password, String username)
			throws MessagingException, NamingException,
			UnsupportedEncodingException {
		MimeMessage mail = new MimeMessage(mailSession);
		mail.setSubject("CGuide - Password");
		Address from = new InternetAddress("noreply@cguide.com",
				"CGuide password system");
		mail.setFrom(from);
		mail.setRecipients(Message.RecipientType.TO,
				javax.mail.internet.InternetAddress.parse(email, false));
		mail.setText("Hi "
				...
				+ "You or someone else has used your email account ("
				+ email
				+ ") to request a new password for your account.\n\n"
