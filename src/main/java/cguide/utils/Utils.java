...
public class Utils {
	...
	public static String generateRandomKey(int length) {
		...
		return key.substring(0, (key.length() > length) ? length
				: key.length() - 1);
	}
	...
	public static Boolean alternateTaskTest(ArrayList<Condition> conditions,
			ArrayList<TriggerCondition> task) {
		...
		for (TriggerCondition tc : task) {
			for (ConditionSet cs : tc.getTriggerConditionSet()) {
				...
				if (flag) {
					System.out.println("Selected according to Condition Set "
							+ cs.getId());
					return true;
				}
			}
	...
	public static Boolean processDecision(ArrayList<Condition> conditions,
			ArrayList<ConditionSet> task) {

		Boolean flag = false;
		for (ConditionSet cs : task) {
			flag = testConditionSet(cs, conditions);
			if (flag) {
				System.out.println("Processed according to Condition Set "
						+ cs.toJson());
				return true;
			}
	...
	private static Boolean testConditionSet(ConditionSet conditionSet,
