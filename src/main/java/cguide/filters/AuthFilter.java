...
public class AuthFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest,
		...
		if (DEV_MODE) {
			...
			httpServletRequest.setAttribute("requestOwner", UserManager
					.getInstance().getUserBeanByID(1L));
			...
			if (authToken != null && authToken.length() > 0) {
				try {
					AutenticacaoBean[] autenticacaoBeans = AutenticacaoManager
							.getInstance().loadByWhere(
									" where auth = '" + authToken + "';");
					if (autenticacaoBeans != null
							&& autenticacaoBeans.length > 0) {
						if (autenticacaoBeans[0].getDuracao()
								.before(new Date())) {
							httpServletResponse
									.sendError(
							...
							httpServletRequest.setAttribute(
									"requestOwner",
									UserManager.getInstance().getUserBeanByID(
											autenticacaoBeans[0]
													.getUtilizadorId()));
						}
