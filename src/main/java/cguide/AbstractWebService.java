...
public abstract class AbstractWebService {
	...
	public void setApplication(Application application) {
		if (application instanceof MainCoreServer) {
			sessionController = ((MainCoreServer) application)
					.getSessionController();
			guidelineHandler = ((MainCoreServer) application)
					.getGuidelineHandler();
		}
	}
	...
	public void setHttpServletRequest(HttpServletRequest httpServletRequest) {
		if (httpServletRequest.getAttribute("requestOwner") != null) {
			requestOwner = (UserBean) httpServletRequest
					.getAttribute("requestOwner");
		}
	}
	...
	protected Boolean isUser() {
		return requestOwner != null
				&& (requestOwner.getType().equals("user") || requestOwner
						.getType().equals("admin"));
	}
}
