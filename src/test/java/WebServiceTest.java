...
/**
 * Created with IntelliJ IDEA. User: tiago Date: 23-07-2013 Time: 13:04 To
...
public class WebServiceTest {
	public static void main(String[] argv) {
		...
		// request.setTaskQuadruple(new TaskQuadruple("Workup1", "Workup1",
		// "172","171"));
		// request.setTaskTriple(new TaskTriple("Workup2", "Workup2", "123"));
		// request.setTaskQuadruple(new TaskQuadruple("GlobalPlanColonCancer",
		// "GlobalPlanColonCancer", "-1", "-1"));
		// request.setTaskQuadruple(new TaskQuadruple("Q02", "Q02",
		// "161","159"));
		TaskTriple tp;
		TaskQuadruple qp;
		Gson gson = new Gson();
		...
		condition
				.setConditionParameter("colon cancer appropriate for resection");
		condition.setComparisonOperator("Equal_to");
		cs.addCondition(condition);
		...
		condition
				.setConditionParameter("suspected or proven metastatic synchronous adenocarcinoma");
		condition.setComparisonOperator("Equal_to");
		cs.addCondition(condition);
		...
		valuePairRequest.add(new BasicNameValuePair("request", gson
				.toJson(request)));
		...
		valuepairTask.add(new BasicNameValuePair("taskFormat",
				"tipo do formato"));
		valuepairTask.add(new BasicNameValuePair("taskDescription",
				"descrição da taska"));
		valuepairTask.add(new BasicNameValuePair("taskIdentifier",
				"identificador da taska"));
		...
		valuepairExam
				.add(new BasicNameValuePair("description", "description2"));
		...
		valuepairFormula.add(new BasicNameValuePair("description",
				"description"));
		valuepairFormula.add(new BasicNameValuePair("identifier", "id"));

		valuepairGuideline.add(new BasicNameValuePair("dateOfCreation",
				"2013-08-02 15:05:16"));
		valuepairGuideline.add(new BasicNameValuePair("dateOfUpdate",
				"2013-08-02 15:05:16"));
		valuepairGuideline.add(new BasicNameValuePair("versionNumber",
				"version number"));
		valuepairGuideline.add(new BasicNameValuePair("guidelineName",
				"parser name"));
		valuepairGuideline.add(new BasicNameValuePair("authorship",
				"tiago Vilas Boas"));
		valuepairGuideline.add(new BasicNameValuePair("identifier",
				"identifier"));

		valuepairMedication.add(new BasicNameValuePair("name", "name"));
		valuepairMedication.add(new BasicNameValuePair("idtask", "1"));
		valuepairMedication.add(new BasicNameValuePair("activeIngredient",
				"active ingredient"));
		valuepairMedication.add(new BasicNameValuePair("dosage", "dosage"));
		valuepairMedication.add(new BasicNameValuePair("pharmaceuticalForm",
				"pharmaceutical form"));
		valuepairMedication.add(new BasicNameValuePair("posology", "posology"));
		valuepairMedication.add(new BasicNameValuePair("description",
				"description"));
		valuepairMedication.add(new BasicNameValuePair("identifier",
				"identifier"));

		valuepairObservation.add(new BasicNameValuePair("parameter",
				"parameter 1"));
		...
		valuepairObservation.add(new BasicNameValuePair("parameterIdentifier",
				"parameterId 1"));
		valuepairObservation.add(new BasicNameValuePair("variableName",
				"variablename 1"));
		valuepairObservation.add(new BasicNameValuePair("parameterValue",
				"parameterValue 1"));
		...
		SendHttpMethod sp = new SendHttpMethod(
				"http://localhost:8080/CGuide_war_exploded/auth", valuePair);
		String result = null;
		String action = null;
		...
		try {
			...
			if (sp.getHttpCode() == 200) {
				...
				System.out
						.println("SOMETHING WENT BOOM or just something wrong with the database");
				System.out.println(result);
		...
		int id = 1;
		// GuidelineList result2 =
		// guidelineList.fromBean(UserManager.getInstance().createUserBean());
		...
		SendHttpMethod sp2 = new SendHttpMethod(
				"http://localhost:8080/CGuide_war_exploded/execution/guideline2/next/",
				valuePairRequest);
		SendHttpMethod update = new SendHttpMethod(
				"http://localhost:8080/CGuide_war_exploded/guideline/update",
				valuePairRequest);
		...
		try {
			...
			Gson gson2 = new GsonBuilder()
					.registerTypeHierarchyAdapter(ClinicalTask.class,
							new ClinicalTaskAdapter())
					.registerTypeHierarchyAdapter(ClinicalAction.class,
							new ClinicalActionAdapter()).setPrettyPrinting()
					.create();
